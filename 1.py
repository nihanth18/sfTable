...
def generate_data():
    ...
    for _ in range(10):
        column_name = fake.word().replace(' ', '_')[:20]  # Generate a simplified column name (up to 20 characters)
    ...
    for column_name in column_names:
        column_values = []
        for _ in range(10):
            column_value = fake.word()  # Generate a random value
...
snowflake_config = {
    'account': 'wgpapyx-perficient_partner',
    'user': 'KNIHANTH',
    'password': 'ThomasShelby@^123',
...
if st.button("Save Data to Snowflake"):
    try:
        ...
        for column_name in generated_data_df.columns:
            create_table_sql += f'{column_name} STRING, '  # Remove double quotes here
        ...
        for _, row in generated_data_df.iterrows():
            ...
            values_sql = "VALUES ("
            for column_name in generated_data_df.columns:
                insert_sql += f'{column_name}, '  # Remove double quotes here
